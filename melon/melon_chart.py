...
def get_melon_chart():
    ...
    for i in chart_list:
        rank.append(i.select_one('.rank').text)         #곡 순위
        title.append(i.select_one('.rank01').a.text)     #곡명
        artist.append(i.select_one('.rank02').a.text)     #가수명
        

    # fianl_list = list(zip(melon_list,get_like_num))
    # print(fianl_list)

    df=pd.DataFrame({'rank':rank, 'artist' : artist,'title' :title ,'likes':get_like_num})      #복수 가수 인 경우 문제 
    print(df)
